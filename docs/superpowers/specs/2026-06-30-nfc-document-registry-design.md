...
`DocumentNFCReader` reads through a new `ChipReading` protocol rather than
calling `PassportReader.readPassport` directly — production wires the real
library implementation; tests inject a fake. This is what makes Tier 2
testing (below) possible without hardware. See Testing for the protocol
shape and fixture approach.
...
Per-country physical hardware testing does not scale as a regression-testing
strategy — it would become an expensive, slow bottleneck on every change if
it were the only tier. Testing is split into three tiers, only the last of
which requires physical hardware:

**Tier 1 — pure logic (CI, every PR, no hardware).** Unit tests for
`MRZParser.parseTD1` (checksum + field-offset cases per seeded country's
known MRZ layout), `DocumentProfile.available(for:)` filtering per build
profile, and TD1/TD3 format auto-detection from OCR'd line geometry. Already
fully automatable, no new infrastructure needed.

**Tier 2 — mocked chip pipeline (CI, every PR, no hardware).** Introduce a
`ChipReading` protocol that `DocumentNFCReader` reads through instead of
calling `PassportReader.readPassport` directly:

```swift
protocol ChipReading {
    func readPassport(mrzKey: String, tags: [DataGroupId],
                       skipSecureElements: Bool, skipCA: Bool,
                       skipPACE: Bool) async throws -> NFCPassportModel
}
```

Production wires the real `PassportReader` from `NFCPassportReader`; tests
inject a fake returning canned `NFCPassportModel`-shaped results assembled
from **synthetic** DG1/DG2/SOD/EF.COM byte fixtures — generated by a
one-time fixture-builder script, not captured from any real person's
document, self-signed under a test DSC/CSCA chain (reusing the existing
self-signed-masterlist mechanism already in `DocumentNFCReader`/
`PassportNFCReader`). This exercises hash-integrity checking
(`passportDataNotTampered`), DG1/DG2 extraction, and the
profile/registry-driven logic end-to-end for every seeded country, entirely
in CI, with no PII and no physical chip involved. Catches the large majority
of regressions (parsing bugs, hash-check bugs, profile-wiring bugs) cheaply.

**Tier 3 — physical hardware-in-the-loop (manual, once per document type,
not per-PR).** Validates what Tier 2 cannot: the actual PACE/BAC/NFC
handshake against a real chip, and whether `dg2Accessible` holds in practice
for each country. Avoiding "go acquire a real person's ID per country" as
the only option:
  - **Official government test-card programs**, where they exist — e.g.
    Germany's BSI TR-03105 conformance test-card set for the
    Personalausweis. Needs per-country research (see follow-up below).
  - **Self-personalized test smartcards** — a programmable JavaCard/JCOP
    card running an open-source eMRTD applet (e.g. JMRTD's applet),
    personalized with the same synthetic Tier 2 fixture data. Lets the
    physical PACE/BAC/NFC path be tested against synthetic data without
    needing any real document or any real person's PII.
  - **Real test documents** as a last resort, with explicit consent, scoped
    to the minimum needed to confirm `dg2Accessible` once per country before
    that `DocumentProfile` ships — mirrors how passport support was
    originally validated.

**Follow-up (not blocking this design):** research which of the seeded
countries (Israel, Germany, France, Portugal, Italy, Spain, Japan, Brazil)
have an official test-card or developer conformance program, to minimize how
often Tier 3's "real test document" fallback is needed.
